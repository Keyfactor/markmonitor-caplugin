...
- `docsource/*.md` — source-of-truth doc fragments (`overview.md`, `architecture.md`,
  ...
  Root `README.md` is **generated by doctool** (`Keyfactor/doctooldotnet`, cloned at
  `~/RiderProjects/doctooldotnet`) from `docsource/` + `integration-manifest.json` — never hand-edit
  `README.md`; edit `docsource/` and regenerate:
  `cd ~/RiderProjects/doctooldotnet && just build && just docs <repo-path> markmonitor-caplugin`.
  Only `configuration.md` (plus its `{% include 'architecture.md' %}`) and the manifest feed the
  README; `overview.md`/`development.md` stay standalone. Images referenced from `configuration.md`
  must use `docsource/images/...`. (`readme_source.md` is a stale placeholder and is not used.)
